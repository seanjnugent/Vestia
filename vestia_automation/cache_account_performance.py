...
def main():
    ...
    try:
        with engine.connect() as conn:
            account_ids = [row[0] for row in conn.execute(text("SELECT distinct account_id FROM public.cash_trade union SELECT distinct account_id FROM public.asset_trade order by 1"))]
