...
const RegualarWithdrawal = ({ onComplete, account }) => {
  ...
  const [proportionalAllocation, setLeaveCash] = useState(false);
  
  const calculateTotalAllocation = () => {
    const total = Object.values(transaction.assetAllocations).reduce((sum, value) => sum + (value || 0), 0);
    return proportionalAllocation ? 100 : total;
  };
  ...
  const renderAssetAllocation = () => (
    <div className="space-y-6">
      ...
      <div className="max-h-64 overflow-y-auto space-y-2">
        {filteredAssets.map(asset => (
          <div
            key={asset.code}
            onClick={() => {
              if (proportionalAllocation) return; // Prevent selection if "Leave as Cash" is active
            ...
            className={`p-4 rounded-xl border-2 cursor-pointer transition-all ${
              proportionalAllocation ? 'opacity-50 cursor-not-allowed' :
                ...
                {selectedAssets.some(a => a.code === asset.code) && (
                  <div className="mt-2">
                    <input
                      ...
                      min="0"
                      max="100"
                      disabled={proportionalAllocation}
      ...
      >
        <div className="flex justify-between items-center">
          <span className="font-medium">Sell from held assets proportionally</span>
          <span className="text-sm text-gray-500">
            {proportionalAllocation ? "100%" : "0%"}
          </span>
        </div>
      </button>
      ...
      <div className="bg-gray-50 p-4 rounded-xl space-y-4">
        ...
        {!proportionalAllocation && selectedAssets.length > 0 && (
        ...
        {proportionalAllocation && (
          <div className="text-sm text-gray-600">
            Assets will be sold based on their proportion within your portfolio to fund your withdrawal.
          </div>
        )}
      </div>
      ...
      <div className="flex justify-between pt-4">
        ...
        <button
          onClick={() => setStage(4)}
          disabled={!proportionalAllocation && calculateTotalAllocation() !== 100}
          className={`px-6 py-3 rounded-xl transition-all ${
            (proportionalAllocation || calculateTotalAllocation() === 100)
              ? "bg-blue-500 text-white hover:bg-blue-600"
              : "bg-gray-300 text-gray-500"
          }`}
        >
...
export default RegualarWithdrawal;